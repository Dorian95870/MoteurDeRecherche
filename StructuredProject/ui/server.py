...
def index():
    ...
    if type(userRequest) != type(None):
        docs = score(app.config['dataSet'], app.config['myIndex'], userRequest)
...
def result_page2():
    ...
    try:
        ...
        for iterator in range(foundBooks['Title'].size):
            TitleAuthorIdArray.append(
                (foundBooksTitles[iterator], foundBooksAuthors[iterator], resultId[iterator]))
